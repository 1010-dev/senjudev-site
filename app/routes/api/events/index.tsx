...
async function fetchConnpassEvents(
  ...
  try {
    ...
    const url = `https://connpass.com/api/v2/events/?group_id=11169&count=${count}&order=${order === 'asc' ? '1' : '2'}`;
    console.log(
...
// フォールバックデータを提供する関数
function getFallbackEvents(): { upcoming: ConnpassResponse; past: ConnpassResponse } {
  return {
    upcoming: {
      events: [],
      results_returned: 0,
...
function categorizeEvents(events: ConnpassEvent[]): {
  ...
  upcoming.sort((a, b) => new Date(a.started_at).getTime() - new Date(b.started_at).getTime());
  past.sort((a, b) => new Date(b.started_at).getTime() - new Date(a.started_at).getTime());
...
export default createRoute(
  ...
  async (c) => {
    try {
      ...
      return c.json({ ...fallbackData, total: fallbackData.upcoming }, {
